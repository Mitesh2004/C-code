...
void Degree(int a[max][max], int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        for (j = 0; j < n; j++)
        {
            incount += a[j][i];  // Count incoming edges
            outcount += a[i][j]; // Count outgoing edges
        }
...
int main()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            AM[i][j] = 0;
            printf("\nIs there an edge from v %d to v %d (YES-1 , NO-0) : ", i + 1, j + 1);
            scanf("%d", &AM[i][j]);
        }
    }
